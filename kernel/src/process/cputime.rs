...
impl ProcessControlBlock {
    ...
    pub fn thread_cputime_ns(&self) -> u64 {
        let (utime, stime) = self.thread_utime_stime_ns();
        utime.saturating_add(stime)
    }

    /// 当前线程（PCB）的 user/system CPU 时间（ns）。
    #[inline]
    pub fn thread_utime_stime_ns(&self) -> (u64, u64) {
        ...
        let ct = self.cputime();
        (
            ct.utime.load(Ordering::Relaxed),
            ct.stime.load(Ordering::Relaxed),
        )
    }
    ...
    pub fn process_cputime_ns(&self) -> u64 {
        let (utime, stime) = self.process_utime_stime_ns();
        utime.saturating_add(stime)
    }

    /// 当前进程（线程组）的 user/system CPU 时间（ns）。
    pub fn process_utime_stime_ns(&self) -> (u64, u64) {
        ...
        if !leader.is_thread_group_leader() {
            ...
            return self.thread_utime_stime_ns();
        }

        let (mut utime, mut stime) = leader.thread_utime_stime_ns();
        let ti = leader.threads_read_irqsave();
        for t in &ti.group_tasks {
            if let Some(p) = t.upgrade() {
                let (thread_utime, thread_stime) = p.thread_utime_stime_ns();
                utime = utime.saturating_add(thread_utime);
                stime = stime.saturating_add(thread_stime);
            }
        }
        (utime, stime)
    }
